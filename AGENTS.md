...
Provide an authenticated API for uploading all posts and reindexing the full list.

Top-level payload fields:

- `posts` (full reindex list)
...
- Each index request is a full reindex list; posts not present in the request must be removed from storage.
- Database row IDs for posts are generated by storage and not supplied by clients.
...
- Authentication for index APIs is mandatory (for example bearer token or signed request).
...
- Emit metrics for index latency, index size growth, and search latency.
